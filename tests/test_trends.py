...
def test_no_fabricated_metrics_and_scoring_normalization(db_session):
    tr = db_session.query(TrendRun).order_by(TrendRun.id.desc()).first()
    if not tr:
...
def test_idempotency_and_stability(db_session):
    run_trend_intelligence()
    runs = db_session.query(TrendRun).order_by(TrendRun.id.desc()).limit(2).all()
    if len(runs) >= 2:
